# Финальная проектная работа курса "Микросервисная архитектура"
...
![диаграмма C1](./C4/C1.png)

#### Уровень контейнеров (C2)

![диаграмма C2](./C4/C2.png)

#### Уровень компонентов (С3)

![диаграмма C3](./C4/C3.png)
...
Проект состоит из пяти заданий. Решение каждого из них нужно залить в отдельную директорию. Создайте в репозитории пять директорий и назовите их «Task1», «Task2», «Task3», «Task4» и «Task5».
...
Доработайте диаграмму C2, отразив на ней изменения, которые вы предлагаете в ландшафте систем компании, связанные с внедрением инструментов защиты (WAF, Firewall и аналогичных). Добавьте текстовое обоснование к схеме с объяснением выбранных механизмов.

В результате у вас должно получиться:

1. Текстовое обоснование выбора региона развёртывания.
2. Схема репликации.
3. Схема реализации геомаршрутизации.
4. Диаграмма аварийного переключения в нотации C4 с основными точками отказа и механизмами переключения, таблица сценариев отказа.
5. Стратегия комплаенса (если вы решили выполнять дополнительное задание).

Обратите внимание, что все изменения нужно выделить красным цветом, чтобы ревьюеру было проще проверить вашу работу.

### Задание 4. Проектируем ценообразование с ML-моделями
...
2. Текстовое описание выбора механизмов, как обеспечить качество данных (если вы решили выполнять дополнительное задание).
...
1. **Понимание задачи и ограничений:** насколько корректно были поняты требования, бизнес-контекст и нефункциональные требования (нагрузка, SLA, безопасность, масштабируемость), а также учтены реальные ограничения (бюджет, сроки и используемые технологии).
2. **Качество архитектурного решения:** соответствие предложенного решения требованиям, баланс между простотой и гибкостью, а также учёт нефункциональных требований.
3. **Аргументация и обоснование:** способность объяснить выбранные решения, ясно обозначить компромиссы (trade-offs) и сравнить альтернативные подходы.
4. **Презентация и визуализация:** насколько ясно и структурированно представлена архитектура, насколько эффективно использованы диаграммы и выделены ключевые элементы без ухода в ненужные детали.
5. **Практичность и реалистичность:** возможность реальной реализации решения, учёт вопросов эксплуатации, мониторинга, CI/CD, а также продуманность рисков и мер их минимизации.
